...
import { AdminAccessLogRequest, AdminAccessLogResponse } from "@shared/api";
...
export const handleAdminAccessLog: RequestHandler = async (req, res) => {
  const { walletAddress, chainName: chainNameRaw, chainId: chainIdRaw } = (req.body || {}) as AdminAccessLogRequest;
  const chainName = String(chainNameRaw || "").trim();
  ...
  if (!walletAddress || !walletAddressRegex.test(walletAddress)) {
    const response: AdminAccessLogResponse = {
      ok: false,
      notified: false,
  ...
  if (!botToken || !chatIds.length) {
    const response: AdminAccessLogResponse = {
      ok: false,
      notified: false,
      message: "Telegram env vars are not configured",
    };
