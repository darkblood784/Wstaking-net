...
/**
 * @title BSCTestnetSecureStakingV5
 * @notice A staking contract specifically designed for a 18â€decimal USDT token on BSC Smart Chain mainnet.
 *         This includes â€œround upâ€ logic so that final rewards never end in an offâ€byâ€1 scenario.
 */
contract BSCTestnetSecureStakingV5 is 
    Initializable,
    OwnableUpgradeable,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public constant MINIMUM_STAKE   = 10 * DECIMAL_FACTOR;
    uint256 public constant THRESHOLD_AMOUNT= 10000 * DECIMAL_FACTOR;
    ...
    // Promotion Durations (use an offset so they donâ€™t conflict)
    ...
    // New mapping for V9 stakes â€“ this is where new stakes will be stored and where migrated stakes will reside.
    ...
    // Upgradable storage â€œgapâ€ (reduced by 1 due to stakeNonce)
    ...
    // We use â€œV1â€ as the final storage
...
*/
    ...
    */
    ...
    function _refreshStakeAfterUpdate(
        ...
        if (effectiveAPR == 0) {
            emit Notification(user, "3::APR error");
            return (0, 0, 0);
        }
        ...
        onlySupportedToken(token)
    {
        ...
        if (effectiveAPR == 0) {
            emit Notification(msg.sender, "3::APR error");
            return;
        }
       ...
       // ----------------------------------------------------------------
    // addFund() Function â€“ Allows additional funds to be added to a stake.
        ...
        onlySupportedToken(token) 
    {
        ...
        // âœ… Calculate and pay unclaimed rewards before resetting the stake
        ...
        if (unclaimed > 0) {
            ...
            if (hwBalance >= unclaimed && hwAllowance >= unclaimed) {
                ...
                bytes32 claimedTxnHash = keccak256(abi.encodePacked(msg.sender, block.timestamp, unclaimed, token));
        ...
        // 4. Early exit if APR error (already handled inside _refreshStakeAfterUpdate but double-safe)
        if (effectiveAPR == 0) {
            return;
        }
    ...
    // ----------------------------------------------------------------
    // claimRewards() Function â€“ Claim available rewards.
    ...
    // ----------------------------------------------------------------
    // unstake() Function â€“ Unstake and claim rewards.
        ...
        onlySupportedToken(token)
    {
        ...
        if (claimable > 0) {
            if (hwBalance >= claimable && hwAllowance >= claimable) {
                ...
                bytes32 claimedTxnHash = keccak256(abi.encodePacked(msg.sender, block.timestamp, claimable, token));
                ...
                // Not enough funds/allowance for rewards: show network congestion and stop
                revert InsufficientHotWalletLiquidity();
            }
        }
        ...
        returns (WalletStakeSummary[] memory summaries)
    {
        ...
        for (uint256 w = 0; w < W; w++) {
            ...
            for (uint256 t = 0; t < T; t++) {
                ...
                // sum this walletâ€™s stakes for this token in the time window
