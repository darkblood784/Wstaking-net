...
/**
 * @title BASETestnetSecureStakingV3
 * @notice A staking contract specifically designed for a 18-decimal USDT token on Base testnet.
 *         This includes “round up” logic so that final rewards never end in an off‐by‐1 scenario.
 */
contract BASETestnetSecureStakingV3 is 
    Initializable,
    OwnableUpgradeable,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    // V3 append-only additions for upgrade-safe layout.
...
/*
    function transferSuperAdmin(address _newSuperAdmin) external onlyOwner {
        require(_newSuperAdmin != address(0), "Invalid address");
        emit SuperAdminTransferred(superAdmin, _newSuperAdmin);
...
*/
    ...
    */
    ...
    function resetTokenAggSeenBatch(address token, address[] calldata users) external onlyAdmin {
    ...
    function rebuildTokenAggBatch(address token, address[] calldata users) external onlyAdmin {
   ...
   // add this near your other admin funcs
    ...
    function adminSyncGlobalsFromTokens(address[] calldata tokens) external onlyAdmin {
        uint256 ts;
        uint256 ta;
        uint256 ur;
        uint256 rg;
