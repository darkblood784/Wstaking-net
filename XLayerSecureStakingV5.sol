...
error UnauthorizedAccess(string message);                     // system disabled or not owner/admin
...
/**
 * @title XLayerSecureStakingV5
 * @notice A staking contract specifically designed for a 6‐decimal USDT token on X‐Layer mainnet.
 *         This includes “round up” logic so that final rewards never end in an off‐by‐1 scenario.
 */
contract XLayerSecureStakingV5 is 
    Initializable,
    OwnableUpgradeable,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    // For 6-decimal USDT, threshold = 10,000 USDT => 10,000 * 10^6 = 1e10
    ...
    // (In V5 these were named "ignoreMaxStakingThreshold" etc. We keep the same names.)
    ...
    mapping(address => mapping(address => StakeInfoV2[])) private multiTokenUserStakesV2;
    ...
    // -----------------------------------------------------------------
    struct StakeInfoV1 {
        uint256 stakedAmount;        // 6-decimals
        uint256 originalStakedAmount;// 6-decimals
        uint256 stakedAt;            // timestamp
        uint256 stakeEnd;            // timestamp
        uint256 rewards;             // total unclaimed (raw integer)
        uint256 originalRewards;     
        uint256 unlockedAt;          // timestamp
        uint256 APR;                 // e.g. 15 => 15%
        uint256 adjustedAPR;         // raw integer if < threshold
        bool claimed;
        bool unstaked;
        uint256 stakingType;         // 0 => no-lock, 1 => locked
        bytes32 txnHash;             // unique for each stake
        bytes32[] claimedTxnHashes;  // claim receipts
        uint256 currentRewards;      // track current calc’d
        uint256 expectedTotalRewards;// from day1 calc
        bytes32 unstakeTxnHash;      // track unstake hash
        uint256 claimedRewards;      // total claimed so far
    }

    // -------------------------------------
    // New Stake Structure (V9) – includes new fields:
    // - version: to track which APR model was applied.
    // - promoId: to identify promo stakes (0 = normal).
    // - isPromo: true if stake was created during a promo period.
    // - addFundAllowed: determines if the user can add more funds.
    // -------------------------------------
    struct StakeInfoV2 {
    ...
    // BSC-hardened additions (append-only for upgrade safety)
    ...
    event SystemStateToggled(bool enabled, string message);
    event HotWalletInsufficientFunds(uint256 available, uint256 required, string message);
    event UpcomingTransactions(address indexed owner, uint256[] amounts, uint256[] dueDates, string message);
    ...
    // Event to track progress of stake migration
    //event StakeMigrationProgress(address indexed user, uint256 indexed stakeIndex, uint256 stakedAmount,  string message);
    // Event to track completion of the batch stake migration
    //event StakeMigrationCompleted(uint256 migratedCount,uint256 batchSize );
    event Fundadded(
        address indexed user,
        address indexed token,
        uint256 oldAmount,
...
/*
    function fixStakeAndDisableAddFund(
        address user,
        address token,
        bytes32 txnHash,
        uint256 index
    ) external onlyOwner {
        // Make sure stake exists at the given index
        multiTokenUserStakesV2[token][user][index]; // (optional safety check)
        
        // Set the mapping so the contract can find the stake
        stakeIndexByHash[user][txnHash] = index;

        // Disable add fund for this stake
        multiTokenUserStakesV2[token][user][index].addFundAllowed = false;
    }


    
    /// @notice Remove duplicate stakes (same txnHash) from V2 for a single user/token
    function dedupeUserStakes(address user, address token) external onlyOwner {
        StakeInfoV2[] storage arr = multiTokenUserStakesV2[token][user];
        uint256 n = arr.length;
        bytes32[] memory seen = new bytes32[](n);
        uint256 write = 0;

        // 1) Compact unique entries to the front of the array
        for (uint256 i = 0; i < n; ++i) {
            bytes32 h = arr[i].txnHash;
            bool exists = false;
            for (uint256 j = 0; j < write; j++) {
                if (seen[j] == h) {
                    exists = true;
                    break;
                }
            ...
            if (!exists) {
                seen[write] = h;
                if (i != write) arr[write] = arr[i];
                write++;
            }
        }

        // 2) Pop off the “tail” duplicates
    ...
    */
    ...
    // Utility: Convert address to string for admin notifications
    ...
    */
    ...
    function upsertOrRenameToken(
        ...
        if (hotWallet_ == address(0)) revert InvalidHotWallet();
        ...
        function adminBackfillTokenAggFromGlobals(address token, bool overwrite) external onlyOwner {
        TokenAgg storage a = _tokenAgg[token];

        bool isZero =
            a.totalStaking == 0 &&
            a.totalActiveStaking == 0 &&
            a.totalRewardsGranted == 0 &&
            a.totalUngrantedRewards == 0 &&
            a.userCount == 0;

        if (!overwrite) require(isZero, "TokenAgg already initialized");

        a.totalStaking          = totalStaking;
        a.totalActiveStaking    = totalActiveStaking;
        a.totalRewardsGranted   = totalRewardsGranted;
        a.totalUngrantedRewards = totalUngrantedRewards;
        a.userCount             = totalStakingUsers;
    }
    */
    ...
    */
    ...
    // ----------------------------------------------------------------
    // Main stake() Function (New Stakes use V1.2 logic)
    // ----------------------------------------------------------------
    function stake(
        address token,
        uint256 durationInMinutes,
        ...
        onlySupportedToken(token)
    {
        ...
        IERC20(token).safeTransfer(tokenHotWallets[token], received);
        ...
        // 13) Build new stake using V1.5 logic.
        StakeInfoV2 memory newStake = StakeInfoV2({
            stakedAmount: amount,
        ...
        multiTokenUserStakesV2[token][msg.sender].push(newStake);
        stakeIndexByHash[msg.sender][txnHash] = multiTokenUserStakesV2[token][msg.sender].length - 1;
        ...
        // Per-token: has this wallet ever staked THIS token?
    ...
    function addFund(
        ...
        onlySupportedToken(token) 
    {
        StakeInfoV2[] storage stakes = multiTokenUserStakesV2[token][msg.sender];
        ...
        StakeInfoV2 storage stakeInfo = stakes[idx];
        ...
        if (IERC20(token).allowance(msg.sender, address(this)) < additionalAmount) {
            emit Notification(msg.sender, "3::Insufficient token allowance");
            return;
        }
        ...
        if (unclaimed != 0) {
            ...
            uint256 hwBalance = IERC20(token).balanceOf(hw);
            uint256 hwAllowance = IERC20(token).allowance(hw, address(this));

            if (hwBalance >= unclaimed && hwAllowance >= unclaimed) {
                IERC20(token).safeTransferFrom(hw, msg.sender, unclaimed);
        ...
        uint256 balanceBefore = IERC20(token).balanceOf(address(this));
        IERC20(token).safeTransferFrom(msg.sender, address(this), additionalAmount);
        uint256 received = IERC20(token).balanceOf(address(this)) - balanceBefore;
        ...
        onlySupportedToken(token) 
    {
        ...
        StakeInfoV2 storage stakeInfo = stakes[stakeIndex];
        ...
        IERC20(token).safeTransferFrom(hw, msg.sender, newClaimable);
        ...
        onlySupportedToken(token)
    {
        ...
        StakeInfoV2 storage st = stakes[foundIndex];
        if (st.unstaked) {
            emit Notification(msg.sender, "3::Already unstaked");
            return;
        }
        ...
        if (claimable != 0) {
            if (hwBalance >= claimable && hwAllowance >= claimable) {
                ...
                IERC20(token).safeTransferFrom(hw, msg.sender, claimable);
        ...
        IERC20(token).safeTransferFrom(hw, msg.sender, totalToReturn);
    ...
    function getUserLockedStakesPaginated(
        ...
        returns (
            uint256 page,
            uint256 limit,
            StakeInfoV2[] memory data,
            uint256 size
        )
    {
        StakeInfoV2[] storage stakes = multiTokenUserStakesV2[token][user];
        ...
        StakeInfoV2[] memory lockedStakes = new StakeInfoV2[](size);
        uint256 index = 0;
        for (uint256 i = 0; i < totalStakes; ++i) {
            if (!stakes[i].unstaked) {
        ...
        for (uint256 i = 0; i < size - 1; ++i) {
            for (uint256 j = 0; j < size - i - 1; j++) {
                if (lockedStakes[j].stakedAt < lockedStakes[j + 1].stakedAt) {
                    StakeInfoV2 memory temp = lockedStakes[j];
                    lockedStakes[j] = lockedStakes[j + 1];
                    lockedStakes[j + 1] = temp;
                }
            }
        }
        StakeInfoV2[] memory stakesPage = new StakeInfoV2[](end - start);
        for (uint256 i = start; i < end; ++i) {
            StakeInfoV2 memory userStake = lockedStakes[i];
        ...
        )
    {
        ...
        StakeInfoV2[] memory tmp = new StakeInfoV2[](unlockedCount);
        uint256 idx = 0;
        for (uint256 i = 0; i < total; ++i) {
            if (stakes[i].unstaked && stakes[i].claimed) {
                tmp[idx] = stakes[i];
        ...
        StakeInfoV2[] memory pageData = new StakeInfoV2[](end - start);
        for (uint256 i = start; i < end; ++i) {
            StakeInfoV2 memory s = tmp[i];
    ...
    */
    ...
    /*
    // Helper struct to hold upcoming stake return info
    struct UpcomingReturn {
        address user;
        address token;
        uint256 stakedAmount;
        uint256 unclaimedRewards; // calculated unclaimed rewards at the time of query
        uint256 totalReturn;      // staked amount plus unclaimed rewards (no penalty assumed)
        uint256 stakeEnd;         // the finishing timestamp of the stake
    }

    /// @notice Returns all active stakes (from all users/tokens) that will finish within the given threshold (in minutes),
    /// and also includes expired stakes that are not yet unstaked.
    
    

    function getAllUpcomingReturns(uint256 timeThresholdInMinutes)
        external
        view
        returns (UpcomingReturn[] memory upcoming,
        uint256 totalReturnSum,
        uint256 distinctUserCount
        )
    {
        uint256 totalUsers = stakingUsersKeys.length;
        uint256 totalTokens = allTokenAddresses.length;
        uint256 count = 0;
        uint256 nowTimestamp = block.timestamp; // Store current time

        // First pass: count how many stakes meet the threshold OR are expired but not unstaked
        for (uint256 i = 0; i < totalUsers; ++i) {
            address user = stakingUsersKeys[i];
            for (uint256 j = 0; j < totalTokens; j++) {
                address token = allTokenAddresses[j];
                StakeInfoV2[] storage stakes = multiTokenUserStakesV2[token][user];
                for (uint256 k = 0; k < stakes.length; k++) {
                    StakeInfoV2 storage st = stakes[k];

                    if (!st.unstaked) { // ✅ Only consider active (not unstaked) stakes
                        uint256 remainingMinutes = (st.stakeEnd > nowTimestamp) 
                            ? (st.stakeEnd - nowTimestamp) / 60 
                            : 0;

                        if (remainingMinutes <= timeThresholdInMinutes || st.stakeEnd < nowTimestamp) {
                            count++;
                        }
                    }
                }
        ...
        UpcomingReturn[] memory results = new UpcomingReturn[](count);
        uint256 index = 0;

        // Second pass: fill in the details
        for (uint256 i = 0; i < totalUsers; ++i) {
            address user = stakingUsersKeys[i];
            for (uint256 j = 0; j < totalTokens; j++) {
                address token = allTokenAddresses[j];
                ...
                for (uint256 k = 0; k < stakes.length; k++) {
                    ...
                    if (!st.unstaked) { // ✅ Only consider active (not unstaked) stakes
                        ...
                        if (remainingMinutes <= timeThresholdInMinutes || st.stakeEnd < nowTimestamp) {
                            ...
                            // Calculate unclaimed rewards based on how long it's been staked
                            uint256 elapsedMin = (nowTimestamp - st.stakedAt) / 60;
                            (uint256 totalAccrued, ) = _calculateRewards(
                                st.APR,
                                st.originalStakedAmount,
                                elapsedMin
                            );
                            if (totalAccrued > st.originalRewards) {
                                totalAccrued = st.originalRewards;
                            ...
                            uint256 unclaimed = 0;
                            if (totalAccrued > st.claimedRewards) {
                                unclaimed = totalAccrued - st.claimedRewards;
                            }
                            if (unclaimed > st.rewards) {
                                unclaimed = st.rewards;
                            }

                            // totalReturn = stakedAmount + unclaimed
                            uint256 totalRet = st.stakedAmount + unclaimed;

                            results[index] = UpcomingReturn({
                                user: user,
                                token: token,
                                stakedAmount: st.stakedAmount,
                                unclaimedRewards: unclaimed,
                                totalReturn: totalRet,
                                stakeEnd: st.stakeEnd
                            });
                            index++;
                        }
                    }
        ...
        // Now we sum up the totalReturn and count distinct users
        uint256 sumOfReturns = 0;
        address[] memory distinctUsers = new address[](count);
        uint256 userCount = 0;

        for (uint256 i = 0; i < results.length; ++i) {
            sumOfReturns += results[i].totalReturn;

            // Check if user is already in distinctUsers
            bool found = false;
            for (uint256 d = 0; d < userCount; d++) {
                if (distinctUsers[d] == results[i].user) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                distinctUsers[userCount] = results[i].user;
                userCount++;
            }
        }

        // Return the array of upcoming AND expired-but-not-unstaked returns
        return (results, sumOfReturns, userCount);
    }
    */
    ...
    function getAllUserStakes(
        address user,
        address token
    ) external view returns (StakeInfoV2[] memory) {
        StakeInfoV2[] storage stakes = multiTokenUserStakesV2[token][user];
        uint256 total = stakes.length;
        ...
        StakeInfoV2[] memory tmp = new StakeInfoV2[](count);
        uint256 idx = 0;
        for (uint256 i = 0; i < total; ++i) {
            if (!stakes[i].unstaked || (stakes[i].unstaked && stakes[i].claimed)) {
                tmp[idx] = stakes[i];
